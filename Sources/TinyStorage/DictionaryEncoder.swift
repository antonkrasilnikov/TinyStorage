...
extension DictionaryEncoder {
    ...
    final class Encoder: Swift.Encoder, EncoderContainer {
        ...
        func container<Key>(keyedBy type: Key.Type) -> KeyedEncodingContainer<Key> where Key: CodingKey {
            ...
            if let old = container as? EncoderKeyContainer {
                storage = old.storage
            }else{
                storage = [:]
            }
            
            let keyed = KeyedContainer<Key>(codingPath: codingPath, userInfo: userInfo, storage: storage)
            container = keyed
        ...
        func unkeyedContainer() -> UnkeyedEncodingContainer {
            ...
            if let old = container as? EncoderUnkeyedContainer {
                storage = old.storage
            }else{
                storage = []
            }
            
            let unkeyed = UnkeyedContainer(codingPath: codingPath, userInfo: userInfo, storage: storage)
            container = unkeyed
            return unkeyed
        }
...
protocol EncoderContainer {
    func toAny() throws -> Any
}

protocol EncoderKeyContainer: EncoderContainer {
    var storage: [String: DictionaryEncoder.Storage] {get}
}

protocol EncoderUnkeyedContainer: EncoderContainer {
    var storage: [DictionaryEncoder.Storage] {get}
}

extension DictionaryEncoder {

    final class KeyedContainer<K: CodingKey>: KeyedEncodingContainerProtocol, EncoderKeyContainer {
        typealias Key = K

        let codingPath: [CodingKey]
        private let userInfo: [CodingUserInfoKey: Any]

        init(codingPath: [CodingKey], userInfo: [CodingUserInfoKey: Any], storage: [String: Storage] = [:]) {
            self.codingPath = codingPath
    ...
    final class UnkeyedContainer: Swift.UnkeyedEncodingContainer, EncoderUnkeyedContainer {

        let codingPath: [CodingKey]
        private let userInfo: [CodingUserInfoKey: Any]

        init(codingPath: [CodingKey], userInfo: [CodingUserInfoKey: Any], storage: [Storage] = []) {
            self.codingPath = codingPath
            self.userInfo = userInfo
            self.storage = storage
        }
